...
class DroneVideoDisplay(QtGui.QMainWindow):
    ...
    def __init__(self):
        ...
        self.subTakePic = rospy.Subscriber('take_pic', String, self.SaveImage)

        self.sub_vicon_data = rospy.Subscriber('/vicon/ARDroneCarre/ARDroneCarre',TransformStamped, self.update_vicon)
    ...
    def SaveImage(self,data):
        ...
        if self.image is not None:
            if self.imageLock.acquire():
                ...
                x = self.sim_translation_x
                y = self.sim_translation_y
                z = self.sim_translation_z
                x_w = self.sim_rotation_x
                y_w = self.sim_rotation_y
                z_w = self.sim_rotation_z
                w_w = self.sim_rotation_w
                pic_name=str(self.image_number_counter)+".jpg"
                file_path="/home/ubuntu16/pics/survey/"
                file_dir = file_path + pic_name
                # Saves image to the file directory chosen.
                cv2.imwrite(file_dir, cv_image)
                img_list="image_list"
                f= open(file_path+img_list+".txt","a+")
                f.write(str(x)+","+str(y)+","+str(z)+","+str(x_w)+","+str(y_w)+","+str(z_w)+","+str(w_w))
                f.write("\n")
                f.close()
                self.image_number_counter=self.image_number_counter+1
    ...
    def update_vicon(self,vicon_data_msg):
        ...
        self.sim_translation_x = vicon_data_msg.transform.translation.x
        self.sim_translation_y = vicon_data_msg.transform.translation.y
        self.sim_translation_z = vicon_data_msg.transform.translation.z
        #Update Current Orientation
        self.sim_rotation_x = vicon_data_msg.transform.rotation.x
        self.sim_rotation_y = vicon_data_msg.transform.rotation.y
        self.sim_rotation_z = vicon_data_msg.transform.rotation.z
        self.sim_rotation_w = vicon_data_msg.transform.rotation.w

        self.sim_current_time=vicon_data_msg.header.stamp
